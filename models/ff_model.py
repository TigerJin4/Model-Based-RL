...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = self(obs,acs,obs_mean, obs_std,acs_mean,acs_std,delta_mean,delta_std)[0] 
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        observations = ptu.from_numpy(observations)
        actions = ptu.from_numpy(actions)
        pred = self(observations,actions,obs_mean, obs_std,acs_mean,acs_std,delta_mean,delta_std)[1]
        loss = self.loss(target,pred)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
