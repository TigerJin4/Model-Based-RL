...
class MPCPolicy(BasePolicy):
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        #       action sequence.
        updated_obs = np.tile(obs, [self.N, 1])
        sum_of_rewards = np.zeros(self.N)
        for t in range(self.horizon):
            updated_obs = model.get_prediction(updated_obs, candidate_action_sequences[:,t,:], self.data_statistics)
            r, _ = self.env.get_reward(updated_obs, candidate_action_sequences[:,t,:])
            sum_of_rewards += r
